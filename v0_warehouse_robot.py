...
class WarehouseRobot:
    ...
    def reset(self, seed=None):
        ...
        while self.obstacle_pos == self.target_pos:
            # Putting a random obstacle
            self.obstacle_pos = [
                random.randint(1, self.grid_rows - 1),
                random.randint(1, self.grid_cols - 1),
            ]
    ...
    def perform_action(self, robot_action: RobotAction) -> bool:
        ...
        def is_obtacle_blocking(robot_action):
            if robot_action == RobotAction.LEFT:
                if (
                    self.robot_pos[1] - 1 == self.obstacle_pos[1]
                    and self.robot_pos[0] == self.obstacle_pos[0]
                ):
                    return True
                ...
                if (
                    self.robot_pos[0] + 1 == self.obstacle_pos[0]
                    and self.robot_pos[1] == self.obstacle_pos[1]
                ):
                    return True
        ...
        if not is_obtacle_blocking(robot_action):
            if robot_action == RobotAction.LEFT:
